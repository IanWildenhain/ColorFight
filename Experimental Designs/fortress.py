...
def earlyGame(game):
    return game.turn < 40
def midGame(game):
    return game.turn > 40 and game.turn < 400
...
def play_game(
    ...
    if game.register(username = username, \
        ...
        while True:
            ...
            if (me.energy >= 150000 or game.turn >= 380):
                gold_rush = True
            ...
            if (gold_rush):
                ...
                for cell in list_mycells:
                    if (not cell.building.is_empty and cell.building.name == "gold_mine" and cell.building.can_upgrade and me.gold >= cell.building.upgrade_gold and cell.natural_gold >= 7):
                    ...
                    if( cell.building.is_empty and me.gold >= 200):
                        ...
                        list_mycells.remove(cell)
                        me.gold -= 200
            elif( not gold_rush and (me.energy_source > me.gold_source * 1.7 and me.energy_source > me.gold_source + 1700)):
                list_mycells.sort(key = sortGold, reverse = True)
                ...
                for cell in list_mycells:
                    ...
                    if( cell.building.is_empty and cell.gold > 7 and cell.gold - cell.energy >= 4 and num_gold_built < 3 and me.gold >= 200):
                        if(midGame(game)):
                            if(cell in set_adj_to_enemy):
                                cmd_list.append(game.build(cell.position, BLD_FORTRESS))
                                set_adj_to_enemy.remove(cell)
                                list_mycells.remove(cell)
                                me.gold -= 200
                        else:
                            cmd_list.append(game.build(cell.position, BLD_GOLD_MINE))
            ...
            for cell in list_mycells:
                if(not gold_rush and not cell.building.is_empty and me.energy < 150000 and cell.building.name == "energy_well" and cell.building.can_upgrade and me.gold >= cell.building.upgrade_gold):
                    ...
                    me.gold -= cell.building.upgrade_gold
                    list_mycells.remove(cell)
                if(cell.building.is_empty and cell.energy >= 3):
            ...
            if (len(list_adj) > 0):
                ...
                if(earlyGame(game)):
                    if( int (list_adj[0].attack_cost + 1) < me.energy):
                            cmd_list.append(game.attack(list_adj[0].position, int(list_adj[0].attack_cost + 1)))
                            me.energy -= int(list_adj[0].attack_cost + 1)
                            my_attack_list.append(list_adj[0].position)

                    for i in range(1, len(list_adj)):
                        cell = list_adj[i]

                        if(int (cell.attack_cost + 1) < me.energy):
                            cmd_list.append(game.attack(cell.position, cell.attack_cost + 1))
                            me.energy -= int(cell.attack_cost + 1)
                            my_attack_list.append(cell.position)
                else:
                    if( int (list_adj[0].attack_cost * 1.05) < me.energy):
                            cmd_list.append(game.attack(list_adj[0].position, int(list_adj[0].attack_cost * 1.05)))
                            me.energy -= int(list_adj[0].attack_cost * 1.05)
                            my_attack_list.append(list_adj[0].position)

                    for i in range(1, len(list_adj)):
                        cell = list_adj[i]

                        if(int (cell.attack_cost * 1.05) < me.energy):
                            cmd_list.append(game.attack(cell.position, int(cell.attack_cost * 1.05)))
                            me.energy -= int(cell.attack_cost *1.05)
                            my_attack_list.append(cell.position)
