...
def play_game(
    ...
    if game.register(username = username, \
            password = password, join_key = 'join'):
        ...
        while True:
            ...
            elif not earlygame(game) and not lategame(game):
                list_adj.sort(key = sortGold, reverse = True)
            ...
            for cell in list_mycells:
                if not(cell.building.is_empty):
                    if cell.building.can_upgrade and \
                        (cell.building.is_home or cell.building.level < me.tech_level) and \
                            ...
                            if earlygame(game) and  cell.building.name == "energy_well" :
                                cmd_list.append(game.upgrade(cell.position))
                                print("We upgraded ({}, {})".format(cell.position.x, cell.position.y))
                                me.gold   -= cell.building.upgrade_gold
                                me.energy -= cell.building.upgrade_energy

                            if not (earlygame(game)) and not (lategame(game)) and cell.building.name == "gold_mine":
                                cmd_list.append(game.upgrade(cell.position))
                                print("We upgraded ({}, {})".format(cell.position.x, cell.position.y))
                                me.gold   -= cell.building.upgrade_gold
                                me.energy -= cell.building.upgrade_energy
                elif True:
                    if not lategame(game) and cell.energy > 5:
                        ...
                        me.gold -= 200
                    if  not earlygame(game) and not lategame(game) and cell.gold > 6:
                ...
                if(list_adj[0].attack_cost < me.energy):
                    cmd_list.append(game.attack(list_adj[0].position, list_adj[0].attack_cost))
                    me.energy -= list_adj[0].attack_cost
